<template>
  <div style="min-height: 100vh">
    ...
    <div class="write-container">
      <div>
        <h3 style="text-align: left; margin-top: 20px">블로그 작성</h3>
      </div>
      <form id="insert-blogPost">
        <div class="left-box">
        ...
        <div class="right-box">
          ...
          <div>
            ...
            <div style="text-align: left">
              <div class="gray-box">
                <v-img :src="thumbnail" class="thumbnail-box" />
              </div>
              <v-flex xs8>
                <v-file-input
                  v-model="image"
...
export default {
  ...
  methods: {
    ...
    insert() {
      ...
      const formData = new FormData(document.getElementById("insert-blogPost"));

      //입력된 태그를 잘라서 #을 붙여서 넣음
      let spliceTag = "";
      for (let i = 0; i < this.tag.length; i++) {
        spliceTag += "#" + this.tag[i];
      }

      //Blob 데이터가 있어서 formData로 넘김
      formData.append("memberId", this.$store.state.userStore.id);
      formData.append("title", this.title);
      formData.append("content", this.content);
      formData.append("blogId", this.$store.state.userStore.id);
      ...
      if (
        ...
        this.$axios
          .post("/api/blogs/insert", formData, {
            Headers: {
          ...
          .then((resp) => {
            ...
            console.log("정보" + this.postInfo.content);
            console.log("번호가 나오남?" + this.postInfo.postNo);
