...
export default {
  ...
  methods: {
    getList() {
      //폴더별 북마크 게시글 리스트
      this.$axios
        .post("/api/bookmarkFolderList", {
          memberId: this.$store.state.userStore.id,
          bookFolderName: this.$route.query.bookFolderName,
          pageNum: this.currentPage,
        })
        .then((resp) => {
          console.log(resp);
          eventBus.$emit("bookmarkName", this.$route.query.bookFolderName);
