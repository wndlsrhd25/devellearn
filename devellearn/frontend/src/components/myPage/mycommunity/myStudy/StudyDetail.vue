<template>
  <div>
        ...
        <div class="card__body">
          <div class="half">
            <div class="featured_text">
              <span class="stock">{{ studyDetail.open }}</span>
              <div class="reviews">
                <span>{{ studyDetail.regDate | yyyyMMdd }}</span>
              </div>
              <h1>{{ studyDetail.title }}</h1>
              <p class="sub">{{ studyDetail.category }}</p>
            </div>
            <div class="image">
              <img :src="'/api/images/teamstudy/' + studyDetail.img" />
            </div>
          </div>
          <div class="half">
            <button
            ...
            <button
              ...
              style="float: right"
              @click="EditButton()"
              v-if="this.$store.state.userStore.id == studyDetail.memberId"
            >
              수정
            </button>
            <button
            ...
            <div class="description">
              <pre style="border:none; font-size: 15px; max-width: 100%; white-space : pre-wrap;">{{ studyDetail.content }}</pre>
            </div>
                  ...
                  <li>
                    ...
                    <div class="commentText">
                      <button
                        ...
                        class="btn04"
                        style="float: right"
                        @click="repEditButton(item.studyrepNo)"
                      ...
                      <button
                        ...
                        class="btn05"
                        style="float: right"
                        @click="repDeleteButton(item.studyrepNo)"
                        v-if="$store.state.userStore.id == item.memberId"
                      >
                        삭제
                      </button>
    ...
    <update-study @reload="reload" ref="updatePopup"></update-study>
  </div>
</template>
...
export default {
  ...
  computed: {
    setParams2() {
      const params2 = {
        studyNo: this.studyDetail.studyNo,
        studyrepNo: this.studyrepNo,
    ...
    setParams() {
      const params = {
        studyNo: this.studyDetail.studyNo,
        memberId: this.studyDetail.memberId,
  ...
  methods: {
    ...
    reportStudy() {
      ...
      this.report.postCode = "studys";
      this.report.postNo = this.studyDetail.studyNo;
    ...
    StudyButton() {
      axios
        .post("/api/studyenter", this.setParams)
        .then((response) => {
          console.log(response);
    ...
    getStudyDetail(studyNo) {
      axios
        .get("/api/study/" + studyNo)
        .then((response) => {
          console.log(response);
          this.studyDetail = response.data;
          this.openDetail = true;
        })
        .catch(function (error) {
          console.log(error);
    ...
    getStudyRepList(studyNo) {
      axios
        .get("/api/studyRep/" + studyNo)
        .then((response) => {
          console.log(response);
    ...
    EditButton() {
      this.openDetail = false;
      this.$refs.updatePopup.studyNo = this.studyDetail.studyNo;
      this.$refs.updatePopup.title = this.studyDetail.title;
      this.$refs.updatePopup.content = this.studyDetail.content;
      this.$refs.updatePopup.category = this.studyDetail.category;
      this.$refs.updatePopup.open = this.studyDetail.open;
      this.$refs.updatePopup.img = this.studyDetail.img;
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .delete("/api/study/" + this.studyDetail.studyNo)
              .then((response) => {
                this.openDetail = false;
                this.$emit("reload");
              })
    ...
    insertrep() {
      if (
        ...
        axios
          .post("/api/studyRep", this.setParams2)
          .then((response) => {
            this.content = "";
            this.getStudyRepList(this.studyDetail.studyNo);
          })
          .catch(function (error) {
            console.log(error);
          });
    ...
    repEditButton(studyrepNo) {
      this.openDetail = false;
      this.$refs.updateRepPopup.studyrepNo = studyrepNo;
      this.$refs.updateRepPopup.content = "";
      this.$refs.updateRepPopup.editMode = true;
      this.$refs.updateRepPopup.openPopup = true;
    },
    repDeleteButton(studyrepNo) {
      axios
        .delete("/api/studyRep/" + studyrepNo)
        .then((response) => {
          this.content = "";
          this.getStudyRepList(this.studyDetail.studyNo);
        })
