...
<div class="card__body">
  <div class="half">
    <div class="featured_text">
      <span class="stock">{{ boardDetail.regDate | yyyyMMdd }}</span>
    <h1 style="font-size:25px">{{ boardDetail.title }}</h1>
    <div class="recommend">
      <p>Write by</p>
      <h3>{{ boardDetail.memberId }}</h3>
    </div>
    </div>
    <button type="button" class="btn01" style="float : left"  @click="reportBoard()"
    v-if="$store.state.userStore.id!=''">신고</button>
    <button type="button" class="btn01" style="float : right" @click="EditButton()"
    v-if="this.$store.state.userStore.id == boardDetail.memberId">수정</button>
    <button type="button" class="btn02" style="float : right" @click="DeleteButton()"
    v-if="this.$store.state.userStore.id == boardDetail.memberId">삭제</button>
    <br><hr>
    <div class="description">
      <pre style="border:none; font-size: 15px; max-width: 100%; white-space : pre-wrap;">{{ boardDetail.content }}</pre>
    </div>
    ...
    <!--댓글-->
      <div class="detailBox">
        ...
        <div class="actionBox">
            <ul class="commentList" v-for="item in repList"
                :key="repList.repNo">
                <li>
                    ...
                    <div class="commentText">
                      <button type="button" class="btn04" style="float:right"
                      @click="repEditButton(item.repNo)"
                      v-if="$store.state.userStore.id == item.memberId"
                      >수정</button>
                      <button type="button" class="btn05" style="float:right"
                      @click="repDeleteButton(item.repNo)"
                      v-if="$store.state.userStore.id == item.memberId"
                      >삭제</button>
          ...
          <div class="form-inline">
              <div class="form-group">
                  <input class="form-control" type="text" placeholder="댓글을 입력하세요" v-model="content" id="textarea-rows" />
              </div>
              <div class="form-group">
...
<div class="card__footer">
    ...
    <update-board @reload="reload" ref="updatePopup"></update-board>
  </div>
</template>
...
export default {
  name: "BoardDetail",
  components: {UpdateBoard, UpdateRep, ReportModal},
  data() {
    return {
      openDetail: false,
      boardDetail: [],
  ...
  methods: {
    ...
    reportBoard() {
      ...
      this.report.postCode = "boards";
      this.report.postNo = this.boardDetail.boardNo;
    ...
    EditButton() {
      this.openDetail = false;
      this.$refs.updatePopup.boardNo = this.boardDetail.boardNo;
      this.$refs.updatePopup.title = this.boardDetail.title;
      this.$refs.updatePopup.content = this.boardDetail.content;
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
            .delete("/api/board/" + this.boardDetail.boardNo)
            .then((response) => {
              this.openDetail = false;
              this.$emit("reload");
            })
    ...
    insertrep() {
      ...
      axios
        ...
        .then((response) => {
          this.content = "";
          this.getRepList(this.boardDetail.boardNo);
        })
        .catch(function (error) {
          console.log(error);
        });
...
.card .card__footer::before {
  ...
  height: 3px;
  background: #00336d;
  background: linear-gradient(to right, #00336d 0%, #00336d 20%, #ddd 20%, #ddd 100%);
}
.recommend {
  flex: 1 0 10px;
}
